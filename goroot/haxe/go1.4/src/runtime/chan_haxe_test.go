...
func TestChan(t *testing.T) {
	...
	for chanCap := 0; chanCap < N; chanCap++ {
		...
		if chanCap > 0 { // NOTE spec does not say that 0 cap chan always blocks...
			...
			if atomic.LoadUint32(&sent) != 0 {
				t.Fatalf("chan[%d]: send to full chan (A)", chanCap)
			}
			// Ensure that non-blocking send does not block.
			select {
			case c <- 0:
				t.Fatalf("chan[%d]: send to full chan (B)", chanCap)
			default:
			}
			<-c
		}
...
func stackGrowthRecursive(sz int) { // Tardisgo version
	if sz > 0 {
		stackGrowthRecursive(sz - 1)
	}
}
